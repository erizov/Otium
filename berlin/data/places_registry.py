# -*- coding: utf-8 -*-
"""Berlin guide registry: flat images/<slug>.jpg paths."""
...
class BerlinExtraImage(TypedDict, total=False):
...
class BerlinPlace(TypedDict, total=False):
    ...
    image_rel_path: str
    image_source_url: str
    additional_images: list[BerlinExtraImage]
    license_note: str
    attribution: str
    address: str
    year_built: str
...
def _load_detail_slugs() -> dict[str, dict]:
    ...
    for path in sorted(base.glob("berlin_place_details*.json")):
...
def _load_places() -> list[BerlinPlace]:
    path = Path(__file__).with_name("berlin_places.json")
    raw: list[dict] = json.loads(path.read_text(encoding="utf-8"))
    raw.extend(load_pdf_expand_rows(Path(__file__).parent, "berlin"))
    raw = drop_empty_place_rows(raw)
    raw = _merge_details(raw)
    raw = merge_second_image_sidecar(raw, Path(__file__).parent, 'berlin')
    return cast(list[BerlinPlace], raw)


BERLIN_PLACES: list[BerlinPlace] = _load_places()
