# -*- coding: utf-8 -*-
"""Florence guide registry: flat images/<slug>.jpg paths."""
...
class FlorenceExtraImage(TypedDict, total=False):
...
class FlorencePlace(TypedDict, total=False):
    ...
    image_rel_path: str
    image_source_url: str
    additional_images: list[FlorenceExtraImage]
    license_note: str
    attribution: str
    address: str
    year_built: str
...
def _load_detail_slugs() -> dict[str, dict]:
    ...
    for path in sorted(base.glob("florence_place_details*.json")):
...
def _load_places() -> list[FlorencePlace]:
    path = Path(__file__).with_name("florence_places.json")
    raw: list[dict] = json.loads(path.read_text(encoding="utf-8"))
    raw.extend(load_pdf_expand_rows(Path(__file__).parent, "florence"))
    raw = drop_empty_place_rows(raw)
    raw = _merge_details(raw)
    raw = merge_second_image_sidecar(raw, Path(__file__).parent, 'florence')
    return cast(list[FlorencePlace], raw)


FLORENCE_PLACES: list[FlorencePlace] = _load_places()
