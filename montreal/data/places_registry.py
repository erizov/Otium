# -*- coding: utf-8 -*-
"""Montreal guide registry: flat images/<slug> paths."""
...
class MontrealExtraImage(TypedDict, total=False):
...
class MontrealPlace(TypedDict, total=False):
    ...
    image_rel_path: str
    image_source_url: str
    additional_images: list[MontrealExtraImage]
    license_note: str
    attribution: str
    address: str
    year_built: str
...
def _load_detail_slugs() -> dict[str, dict]:
    ...
    for path in sorted(base.glob("montreal_place_details*.json")):
...
def _load_places() -> list[MontrealPlace]:
    path = Path(__file__).with_name("montreal_places.json")
    raw: list[dict] = json.loads(path.read_text(encoding="utf-8"))
    raw.extend(load_pdf_expand_rows(Path(__file__).parent, "montreal"))
    raw = drop_empty_place_rows(raw)
    raw = _merge_details(raw)
    raw = merge_second_image_sidecar(raw, Path(__file__).parent, 'montreal')
    return cast(list[MontrealPlace], raw)


MONTREAL_PLACES: list[MontrealPlace] = _load_places()
