# -*- coding: utf-8 -*-
"""Prague guide registry: flat images/<slug>.jpg paths."""
...
class PragueExtraImage(TypedDict, total=False):
...
class PraguePlace(TypedDict, total=False):
    ...
    image_rel_path: str
    image_source_url: str
    additional_images: list[PragueExtraImage]
    license_note: str
    attribution: str
    address: str
    year_built: str
...
def _load_detail_slugs() -> dict[str, dict]:
    ...
    for path in sorted(base.glob("prague_place_details*.json")):
...
def _load_places() -> list[PraguePlace]:
    path = Path(__file__).with_name("prague_places.json")
    raw: list[dict] = json.loads(path.read_text(encoding="utf-8"))
    raw.extend(load_pdf_expand_rows(Path(__file__).parent, "prague"))
    raw = drop_empty_place_rows(raw)
    raw = _merge_details(raw)
    raw = merge_second_image_sidecar(raw, Path(__file__).parent, 'prague')
    return cast(list[PraguePlace], raw)


PRAGUE_PLACES: list[PraguePlace] = _load_places()
