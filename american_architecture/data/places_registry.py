...
def _load_places() -> list[CityPlace]:
    path = Path(__file__).with_name("american_architecture_places.json")
    data_dir = Path(__file__).parent
    raw: list[dict] = json.loads(path.read_text(encoding="utf-8"))
    raw.extend(load_pdf_expand_rows(data_dir, "american_architecture"))
    return cast(list[CityPlace], raw)


AMERICAN_ARCHITECTURE_PLACES: list[CityPlace] = _load_places()
