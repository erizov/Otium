...
def _load_places() -> list[CityPlace]:
    path = Path(__file__).with_name("french_architecture_places.json")
    data_dir = Path(__file__).parent
    raw: list[dict] = json.loads(path.read_text(encoding="utf-8"))
    raw.extend(load_pdf_expand_rows(data_dir, "french_architecture"))
    return cast(list[CityPlace], raw)


FRENCH_ARCHITECTURE_PLACES: list[CityPlace] = _load_places()
