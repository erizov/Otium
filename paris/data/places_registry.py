# -*- coding: utf-8 -*-
"""Paris guide registry: flat images/<slug>.jpg paths."""
...
class ParisExtraImage(TypedDict, total=False):
...
class ParisPlace(TypedDict, total=False):
    ...
    image_rel_path: str
    image_source_url: str
    additional_images: list[ParisExtraImage]
    license_note: str
    attribution: str
    address: str
    year_built: str
...
def _load_detail_slugs() -> dict[str, dict]:
    ...
    for path in sorted(base.glob("paris_place_details*.json")):
...
def _load_places() -> list[ParisPlace]:
    path = Path(__file__).with_name("paris_places.json")
    raw: list[dict] = json.loads(path.read_text(encoding="utf-8"))
    raw.extend(load_pdf_expand_rows(Path(__file__).parent, "paris"))
    raw = drop_empty_place_rows(raw)
    raw = _merge_details(raw)
    raw = merge_second_image_sidecar(raw, Path(__file__).parent, 'paris')
    return cast(list[ParisPlace], raw)


PARIS_PLACES: list[ParisPlace] = _load_places()
