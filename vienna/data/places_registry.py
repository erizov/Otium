# -*- coding: utf-8 -*-
"""Vienna guide registry: flat images/<slug> paths."""
...
class ViennaExtraImage(TypedDict, total=False):
...
class ViennaPlace(TypedDict, total=False):
    ...
    image_rel_path: str
    image_source_url: str
    additional_images: list[ViennaExtraImage]
    license_note: str
    attribution: str
    address: str
    year_built: str
...
def _load_detail_slugs() -> dict[str, dict]:
    ...
    for path in sorted(base.glob("vienna_place_details*.json")):
...
def _load_places() -> list[ViennaPlace]:
    path = Path(__file__).with_name("vienna_places.json")
    raw: list[dict] = json.loads(path.read_text(encoding="utf-8"))
    raw.extend(load_pdf_expand_rows(Path(__file__).parent, "vienna"))
    raw = drop_empty_place_rows(raw)
    raw = _merge_details(raw)
    raw = merge_second_image_sidecar(raw, Path(__file__).parent, 'vienna')
    return cast(list[ViennaPlace], raw)


VIENNA_PLACES: list[ViennaPlace] = _load_places()
