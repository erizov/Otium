...
def _load_detail_slugs() -> dict[str, dict]:
    ...
    for path in sorted(base.glob("amsterdam_place_details*.json")):
...
def _load_places() -> list[CityPlace]:
    path = Path(__file__).with_name("amsterdam_places.json")
    raw: list[dict] = json.loads(path.read_text(encoding="utf-8"))
    raw.extend(load_pdf_expand_rows(Path(__file__).parent, "amsterdam"))
    raw = drop_empty_place_rows(raw)
    raw = _merge_details(raw)
    raw = merge_second_image_sidecar(raw, Path(__file__).parent, 'amsterdam')
    return cast(list[CityPlace], raw)


AMSTERDAM_PLACES: list[CityPlace] = _load_places()
