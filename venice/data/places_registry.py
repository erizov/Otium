# -*- coding: utf-8 -*-
"""Venice guide registry: flat images/<slug>.jpg paths."""
...
class VeniceExtraImage(TypedDict, total=False):
...
class VenicePlace(TypedDict, total=False):
    ...
    image_rel_path: str
    image_source_url: str
    additional_images: list[VeniceExtraImage]
    license_note: str
    attribution: str
    address: str
    year_built: str
...
def _load_detail_slugs() -> dict[str, dict]:
    ...
    for path in sorted(base.glob("venice_place_details*.json")):
...
def _load_places() -> list[VenicePlace]:
    path = Path(__file__).with_name("venice_places.json")
    raw: list[dict] = json.loads(path.read_text(encoding="utf-8"))
    raw.extend(load_pdf_expand_rows(Path(__file__).parent, "venice"))
    raw = drop_empty_place_rows(raw)
    raw = _merge_details(raw)
    raw = merge_second_image_sidecar(raw, Path(__file__).parent, 'venice')
    return cast(list[VenicePlace], raw)


VENICE_PLACES: list[VenicePlace] = _load_places()
