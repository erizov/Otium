# -*- coding: utf-8 -*-
"""Primary + second image copy for Russian Architecture guide."""
...
def city_additional_sources(
    ...
    """On-disk additional images from a city place (at most one)."""
...
def attach_additional_image_rows(
    ...
    """Add ``additional_images`` metadata when a city place has a second photo."""
    if not city_row:
        return
...
def link_additional_images(
    ...
    """Copy second image(s) from city folders. Returns count linked."""
    slug = str(row.get("slug") or "").strip()
    if not slug:
        return 0
    linked = 0
...
def prune_missing_additional_images(
    ...
    """Drop ``additional_images`` entries with no local file."""
    extras = row.get("additional_images")
    if not isinstance(extras, list):
        return
