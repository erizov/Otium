# -*- coding: utf-8 -*-
"""Budapest guide registry: flat images/<slug>.jpg paths."""
...
class BudapestExtraImage(TypedDict, total=False):
...
class BudapestPlace(TypedDict, total=False):
    ...
    image_rel_path: str
    image_source_url: str
    additional_images: list[BudapestExtraImage]
    license_note: str
    attribution: str
    address: str
    year_built: str
...
def _load_detail_slugs() -> dict[str, dict]:
    ...
    for path in sorted(base.glob("budapest_place_details*.json")):
...
def _load_places() -> list[BudapestPlace]:
    path = Path(__file__).with_name("budapest_places.json")
    raw: list[dict] = json.loads(path.read_text(encoding="utf-8"))
    raw.extend(load_pdf_expand_rows(Path(__file__).parent, "budapest"))
    raw = drop_empty_place_rows(raw)
    raw = _merge_details(raw)
    raw = merge_second_image_sidecar(raw, Path(__file__).parent, 'budapest')
    return cast(list[BudapestPlace], raw)


BUDAPEST_PLACES: list[BudapestPlace] = _load_places()
