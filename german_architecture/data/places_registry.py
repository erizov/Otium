...
def _load_places() -> list[CityPlace]:
    path = Path(__file__).with_name("german_architecture_places.json")
    data_dir = Path(__file__).parent
    raw: list[dict] = json.loads(path.read_text(encoding="utf-8"))
    raw.extend(load_pdf_expand_rows(data_dir, "german_architecture"))
    return cast(list[CityPlace], raw)


GERMAN_ARCHITECTURE_PLACES: list[CityPlace] = _load_places()
