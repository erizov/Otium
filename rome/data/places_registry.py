# -*- coding: utf-8 -*-
"""Rome guide registry: flat images/<slug>.jpg paths."""
...
class RomeExtraImage(TypedDict, total=False):
...
class RomePlace(TypedDict, total=False):
    ...
    image_rel_path: str
    image_source_url: str
    additional_images: list[RomeExtraImage]
    license_note: str
    attribution: str
    address: str
    year_built: str
...
def _load_detail_slugs() -> dict[str, dict]:
    ...
    for path in sorted(base.glob("rome_place_details*.json")):
...
def _load_places() -> list[RomePlace]:
    path = Path(__file__).with_name("rome_places.json")
    raw: list[dict] = json.loads(path.read_text(encoding="utf-8"))
    raw.extend(load_pdf_expand_rows(Path(__file__).parent, "rome"))
    raw = drop_empty_place_rows(raw)
    raw = _merge_details(raw)
    raw = merge_second_image_sidecar(raw, Path(__file__).parent, 'rome')
    return cast(list[RomePlace], raw)


ROME_PLACES: list[RomePlace] = _load_places()
