        ...
        for path in sorted(data_dir.glob("*places*.json")):
            ...
            for row in rows:
                ...
                index[key] = {
                    ...
                    "name_en": str(row.get("subtitle_en") or row.get("name_en") or ""),
                    "image_rel_path": rel,
                    "image_source_url": str(row.get("image_source_url") or ""),
                    "history_ru": str(row.get("history_ru") or row.get("history") or ""),
                    "history_en": str(row.get("history_en") or row.get("history") or ""),
                    "significance_ru": str(
                        row.get("significance_ru") or row.get("significance") or "",
                    ),
                    "significance_en": str(
                        row.get("significance_en") or row.get("significance") or "",
                    ),
