# -*- coding: utf-8 -*-
"""Boston guide registry: flat images/<slug> paths."""
...
class BostonExtraImage(TypedDict, total=False):
...
class BostonPlace(TypedDict, total=False):
    ...
    image_rel_path: str
    image_source_url: str
    additional_images: list[BostonExtraImage]
    license_note: str
    attribution: str
    address: str
    year_built: str
...
def _load_detail_slugs() -> dict[str, dict]:
    ...
    for path in sorted(base.glob("boston_place_details*.json")):
...
def _load_places() -> list[BostonPlace]:
    path = Path(__file__).with_name("boston_places.json")
    raw: list[dict] = json.loads(path.read_text(encoding="utf-8"))
    raw.extend(load_pdf_expand_rows(Path(__file__).parent, "boston"))
    raw = drop_empty_place_rows(raw)
    raw = _merge_details(raw)
    raw = merge_second_image_sidecar(raw, Path(__file__).parent, 'boston')
    return cast(list[BostonPlace], raw)


BOSTON_PLACES: list[BostonPlace] = _load_places()
