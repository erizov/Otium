# -*- coding: utf-8 -*-
"""Download Jerusalem guide images from image_source_url (flat images/)."""
...
from jerusalem.whitelist import default_whitelist_path, url_is_whitelisted
...
_USER_AGENT = (
    "ExcursionGuide-Jerusalem/1.0 (batch download for local guide; "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download Jerusalem guide images into jerusalem/images/.",
    )
    parser.add_argument(
        "--jerusalem-root",
        type=Path,
    ...
    root = args.jerusalem_root.resolve()
    ...
    if not todo:
        n = len(JERUSALEM_PLACES)
        if n == 0:
            print(
                "No entries in JERUSALEM_PLACES — add jerusalem_places.json.",
                file=sys.stderr,
            )
