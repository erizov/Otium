...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download dublin guide images.",
    )
    parser.add_argument(
        "--dublin-root",
        type=Path,
        default=_PROJECT_ROOT / "dublin",
        dest="city_root",
        help="dublin tree root",
    )
    add_download_image_args(parser)
    args = parser.parse_args()
    return download_jerusalem_style_images(
        city_root=args.city_root,
        places=DUBLIN_PLACES,
        whitelist_path=default_whitelist_path(),
        title_page_assets=title_page_assets_for_download_arg("dublin"),
        args=args,
