# -*- coding: utf-8 -*-
"""Download Philadelphia guide images (flat images/)."""
...
_TITLE_PAGE_ASSETS: tuple[tuple[str, str], ...] = (
    (
        ...
        "Seal_of_Philadelphia%2C_Pennsylvania.svg",
    ),
    (
        "images/guide_flag.svg",
        "https://upload.wikimedia.org/wikipedia/commons/5/5d/"
        "Flag_of_Philadelphia%2C_Pennsylvania.svg",
    ),
)
_USER_AGENT = (
    "ExcursionGuide-Philadelphia/1.0 (batch download for local guide; "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Download Philadelphia guide images into philadelphia/images/."
        ),
    )
    parser.add_argument(
        "--philadelphia-root",
    ...
    root = args.philadelphia_root.resolve()
    ...
    for place in PHILADELPHIA_PLACES:
        slug = place.get("slug", "?")
    ...
    if not todo:
        n = len(PHILADELPHIA_PLACES)
        if n == 0:
            print(
                "No entries in PHILADELPHIA_PLACES — add "
                "philadelphia_places.json.",
                file=sys.stderr,
            )
