# -*- coding: utf-8 -*-
"""HTML + PDF for Madrid from files under madrid/images/."""
...
from madrid.data.places_registry import MADRID_PLACES, MadridPlace
...
MADRID_HTML_NAME = "madrid_guide.html"
MADRID_PDF_NAME = "madrid_guide.pdf"

_TITLE_SYMBOLS: tuple[tuple[str, str], ...] = (
    (
        "images/guide_coat_of_arms.svg",
        "Coat of arms of Madrid (city, Wikimedia Commons)",
    ),
    (
        "images/guide_flag.svg",
        "Flag of the Community of Madrid (Wikimedia Commons)",
    ),
)
...
def _place_has_displayable_body(p: MadridPlace) -> bool:
...
def _places_with_local_images(root: Path) -> list[MadridPlace]:
    out: list[MadridPlace] = []
    for p in MADRID_PLACES:
...
def _place_meta_line(p: MadridPlace) -> str | None:
    parts: list[str] = []
...
def _image_srcs_for_place(root: Path, p: MadridPlace) -> list[str]:
    if p.get("suppress_images_for_pdf"):
        return []
    out: list[str] = []
    rel = p.get("image_rel_path")
...
def _place_block(p: MadridPlace, img_srcs: list[str]) -> str:
    name_en = escape(p.get("name_en", ""))
    name_plain = p.get("name_en", "")
    sub_es = p.get("subtitle_es", "")
    sub_html = (
        '<p class="sub-es">{}</p>'.format(escape(sub_es))
...
def _heraldry_html(root: Path) -> str:
    chunks: list[str] = [
        '<div class="madrid-title-symbols" '
        'aria-label="Madrid city arms and regional flag">',
...
def _build_html(root: Path, places: list[MadridPlace]) -> str:
    font_href, _, _ = typography_triple("madrid")
    blocks: list[str] = [_cover_otium_html()]
    blocks.append(
        '<header class="guide-title">'
        "{}"
        "<h1 class=\"guide-title-main\">Madrid</h1>"
    ...
    body_inner = "\n".join(blocks)
    css = guide_inline_css("madrid-title-symbols", "madrid")
    return (
        "<!DOCTYPE html>\n"
        '<html lang="en">\n<head>\n'
        '<meta charset="utf-8"/>\n'
        "<title>Guide · Madrid · OTIUM</title>\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build Madrid HTML/PDF using files under madrid/images."
        ),
    )
    parser.add_argument(
        "--madrid-root",
        type=Path,
        default=_PROJECT_ROOT / "madrid",
        help="Madrid tree root (default: madrid/)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Output directory (default: madrid/output/)",
    )
    parser.add_argument(
        "--html-only",
        action="store_true",
    ...
    madrid_root = args.madrid_root.resolve()
    out_dir = (
        args.output_dir.resolve()
        if args.output_dir
        else madrid_root / "output"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    places = _places_with_local_images(madrid_root)
    if not places:
        print(
            "No places with local images (>= {} bytes). "
            "Fill madrid_places.json and run "
            "scripts/download_madrid_images.py.".format(MIN_IMAGE_BYTES),
            file=sys.stderr,
        )
        return 2

    html_path = out_dir / MADRID_HTML_NAME
    html_path.write_text(_build_html(madrid_root, places), encoding="utf-8")
    ...
    pdf_path = out_dir / MADRID_PDF_NAME
    footer = (
