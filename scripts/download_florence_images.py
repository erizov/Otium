# -*- coding: utf-8 -*-
"""Download Florence guide images from image_source_url (flat images/)."""
...
from florence.whitelist import default_whitelist_path, url_is_whitelisted
...
_TITLE_PAGE_ASSETS: tuple[tuple[str, str], ...] = (
    (
        ...
        "Stemma_del_Comune_di_Firenze.svg",
    ),
    (
        "images/guide_flag.svg",
        "https://upload.wikimedia.org/wikipedia/commons/a/a4/"
        "Flag_of_Florence.svg",
    ),
)
_USER_AGENT = (
    "ExcursionGuide-Florence/1.0 (batch download for local guide; "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download Florence guide images into florence/images/.",
    )
    parser.add_argument(
        "--florence-root",
        type=Path,
    ...
    root = args.florence_root.resolve()
    ...
    if not todo:
        n = len(FLORENCE_PLACES)
        if n == 0:
            print(
                "No entries in FLORENCE_PLACES — add florence_places.json.",
                file=sys.stderr,
            )
