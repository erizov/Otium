# -*- coding: utf-8 -*-
"""Проверка image_source_url у объектов SPB против whitelist."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate SPB place image_source_url against SOURCES_WHITELIST.md.",
    )
    parser.add_argument(
        "--whitelist",
        type=Path,
    ...
    errors: list[str] = []
    for i, place in enumerate(SPB_PLACES):
        ...
        for j, extra in enumerate(place.get("additional_images") or [], start=1):
            ...
            if not eu:
                errors.append(
                    "{}: additional #{} missing image_source_url".format(slug, j),
                )
                continue
            if not url_is_whitelisted(eu, whitelist_path=wpath):
                errors.append(
                    "{}: additional #{} not whitelisted: {}".format(slug, j, eu),
                )

    if errors:
        print("SPB source URL validation failed:", file=sys.stderr)
        for line in errors:
