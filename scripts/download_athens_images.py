...
from athens.data.places_registry import ATHENS_PLACES
from athens.whitelist import default_whitelist_path
from athens.whitelist import url_is_whitelisted
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download athens guide images.",
    )
    parser.add_argument(
        "--athens-root",
        type=Path,
        default=_PROJECT_ROOT / "athens",
        dest="city_root",
        help="athens tree root",
    )
    add_download_image_args(parser)
    args = parser.parse_args()
    return download_jerusalem_style_images(
        city_root=args.city_root,
        places=ATHENS_PLACES,
        whitelist_path=default_whitelist_path(),
        title_page_assets=title_page_assets_for_download_arg("athens"),
        args=args,
