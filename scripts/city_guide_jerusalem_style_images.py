# -*- coding: utf-8 -*-
"""Shared image download + whitelist validation for Jerusalem-style guides."""
...
def download_jerusalem_style_images(
    *,
    city_root: Path,
...
def add_download_image_args(p: argparse.ArgumentParser) -> None:
    p.add_argument(
        "--force",
        action="store_true",
...
def validate_jerusalem_style_urls(
    places: Sequence[dict[str, Any]],
    *,
    whitelist_path: Path,
    url_is_whitelisted_fn: Callable[..., bool],
    ...
    """Validate all image_source_url values. Returns 0 if OK."""
    if not whitelist_path.is_file():
        print("Whitelist file missing: {}".format(whitelist_path), file=sys.stderr)
        return 2
    errors: list[str] = []
    for place in places:
        ...
        if not url:
            ...
            continue
        if not url_is_whitelisted_fn(url, whitelist_path=whitelist_path):
        ...
        for j, extra in enumerate(place.get("additional_images") or [], start=1):
            ...
            if not eu:
                ...
                continue
            if not url_is_whitelisted_fn(eu, whitelist_path=whitelist_path):
                errors.append(
    ...
    if errors:
        print("{} source URL validation failed:".format(city_label), file=sys.stderr)
        for line in errors:
            print("  {}".format(line), file=sys.stderr)
        return 1
    print(
        "OK: {} place(s), all image_source_url whitelisted.".format(len(places)),
    )
    return 0
