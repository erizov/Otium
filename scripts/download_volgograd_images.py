# -*- coding: utf-8 -*-
"""Download Volgograd guide images."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download Volgograd guide images.",
    )
    parser.add_argument(
        "--volgograd-root",
        type=Path,
        default=_PROJECT_ROOT / "volgograd",
        dest="city_root",
        help="volgograd tree root",
    )
    add_download_image_args(parser)
    args = parser.parse_args()
    return download_jerusalem_style_images(
        city_root=args.city_root,
        places=VOLGOGRAD_PLACES,
        whitelist_path=default_whitelist_path(),
        title_page_assets=title_page_assets_for_download_arg("volgograd"),
        args=args,
