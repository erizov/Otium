# -*- coding: utf-8 -*-
"""Download Chernivtsi guide images."""
...
from chernivtsi.whitelist import default_whitelist_path
from chernivtsi.whitelist import url_is_whitelisted
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download Chernivtsi guide images.",
    )
    parser.add_argument(
        "--chernivtsi-root",
        type=Path,
        default=_PROJECT_ROOT / "chernivtsi",
        dest="city_root",
        help="chernivtsi tree root",
    )
    add_download_image_args(parser)
    args = parser.parse_args()
    return download_jerusalem_style_images(
        city_root=args.city_root,
        places=CHERNIVTSI_PLACES,
        whitelist_path=default_whitelist_path(),
        title_page_assets=title_page_assets_for_download_arg("chernivtsi"),
        args=args,
