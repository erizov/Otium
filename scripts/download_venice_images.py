# -*- coding: utf-8 -*-
"""Download Venice guide images from image_source_url (flat images/)."""
...
from venice.whitelist import default_whitelist_path, url_is_whitelisted
...
_TITLE_PAGE_ASSETS: tuple[tuple[str, str], ...] = (
    (
        ...
        "Coat_of_Arms_of_the_Republic_of_Venice.svg",
    ),
    (
        "images/guide_flag.svg",
        "https://upload.wikimedia.org/wikipedia/commons/0/01/"
        "Flag_of_the_Republic_of_Venice.svg",
    ),
)
_USER_AGENT = (
    "ExcursionGuide-Venice/1.0 (batch download for local guide; "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download Venice guide images into venice/images/.",
    )
    parser.add_argument(
        "--venice-root",
        type=Path,
    ...
    if not todo:
        ...
        if n == 0:
            print(
                "No entries in VENICE_PLACES — add venice_places.json.",
                file=sys.stderr,
            )
