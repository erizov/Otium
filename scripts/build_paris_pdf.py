# -*- coding: utf-8 -*-
"""HTML + PDF for Paris from files under paris/images/."""
...
from paris.data.places_registry import PARIS_PLACES, ParisPlace
...
PARIS_HTML_NAME = "paris_guide.html"
PARIS_PDF_NAME = "paris_guide.pdf"

_TITLE_SYMBOLS: tuple[tuple[str, str], ...] = (
    (
        "images/guide_coat_of_arms.svg",
        "Coat of arms of Paris (Wikimedia Commons)",
    ),
    (
        "images/guide_flag.svg",
        "Flag of Paris (Wikimedia Commons)",
    ),
)
...
def _place_has_displayable_body(p: ParisPlace) -> bool:
...
def _places_with_local_images(root: Path) -> list[ParisPlace]:
    out: list[ParisPlace] = []
...
def _place_meta_line(p: ParisPlace) -> str | None:
    parts: list[str] = []
...
def _image_srcs_for_place(root: Path, p: ParisPlace) -> list[str]:
    if p.get("suppress_images_for_pdf"):
        return []
    out: list[str] = []
    rel = p.get("image_rel_path")
...
def _place_block(p: ParisPlace, img_srcs: list[str]) -> str:
...
def _heraldry_html(root: Path) -> str:
    chunks: list[str] = [
        '<div class="paris-title-symbols" '
        'aria-label="Paris coat of arms and flag">',
...
def _build_html(root: Path, places: list[ParisPlace]) -> str:
    font_href, _, _ = typography_triple("paris")
    blocks: list[str] = [_cover_otium_html()]
    blocks.append(
        '<header class="guide-title">'
        "{}"
        "<h1 class=\"guide-title-main\">Paris</h1>"
    ...
    body_inner = "\n".join(blocks)
    css = guide_inline_css("paris-title-symbols", "paris")
    return (
        "<!DOCTYPE html>\n"
        '<html lang="en">\n<head>\n'
        '<meta charset="utf-8"/>\n'
        "<title>Guide · Paris · OTIUM</title>\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build Paris HTML/PDF using files under paris/images."
        ),
    )
    parser.add_argument(
        "--paris-root",
        type=Path,
        default=_PROJECT_ROOT / "paris",
        help="Paris tree root (default: paris/)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Output directory (default: paris/output/)",
    )
    parser.add_argument(
        "--html-only",
        action="store_true",
    ...
    paris_root = args.paris_root.resolve()
    out_dir = (
        args.output_dir.resolve()
        if args.output_dir
        else paris_root / "output"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    places = _places_with_local_images(paris_root)
    if not places:
        print(
            "No places with local images (>= {} bytes). "
            "Fill paris_places.json and run "
            "scripts/download_paris_images.py.".format(MIN_IMAGE_BYTES),
            file=sys.stderr,
        )
        return 2

    html_path = out_dir / PARIS_HTML_NAME
    html_path.write_text(_build_html(paris_root, places), encoding="utf-8")
    ...
    pdf_path = out_dir / PARIS_PDF_NAME
    footer = (
