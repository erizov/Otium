...
from budapest.data.places_registry import BUDAPEST_PLACES, BudapestPlace
...
BUDAPEST_HTML_NAME = "budapest_guide.html"
BUDAPEST_PDF_NAME = "budapest_guide.pdf"

_TITLE_SYMBOLS: tuple[tuple[str, str], ...] = (
    (
        "images/guide_coat_of_arms.svg",
        "Coat of arms of Budapest (Wikimedia Commons)",
    ),
    (
        "images/guide_flag.svg",
        "Flag of Budapest (Wikimedia Commons)",
    ),
)
...
def _place_has_displayable_body(p: BudapestPlace) -> bool:
...
def _places_with_local_images(root: Path) -> list[BudapestPlace]:
    out: list[BudapestPlace] = []
    for p in BUDAPEST_PLACES:
...
def _place_meta_line(p: BudapestPlace) -> str | None:
    parts: list[str] = []
...
def _image_srcs_for_place(root: Path, p: BudapestPlace) -> list[str]:
    if p.get("suppress_images_for_pdf"):
        return []
    out: list[str] = []
    rel = p.get("image_rel_path")
...
def _place_block(p: BudapestPlace, img_srcs: list[str]) -> str:
    name_en = escape(p.get("name_en", ""))
    name_plain = p.get("name_en", "")
    sub_hu = p.get("subtitle_hu", "")
    sub_html = (
        '<p class="sub-hu">{}</p>'.format(escape(sub_hu))
        if is_substantive_text(sub_hu)
        else ""
    )
...
def _heraldry_html(root: Path) -> str:
    chunks: list[str] = [
        '<div class="budapest-title-symbols" '
        'aria-label="Budapest coat of arms and flag">',
...
def _build_html(root: Path, places: list[BudapestPlace]) -> str:
    font_href, _, _ = typography_triple("budapest")
    blocks: list[str] = [_cover_otium_html()]
    blocks.append(
        '<header class="guide-title">'
        "{}"
        "<h1 class=\"guide-title-main\">Budapest</h1>"
    ...
    body_inner = "\n".join(blocks)
    css = guide_inline_css("budapest-title-symbols", "budapest")
    return (
        "<!DOCTYPE html>\n"
        '<html lang="en">\n<head>\n'
        '<meta charset="utf-8"/>\n'
        "<title>Guide · Budapest · OTIUM</title>\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build Budapest HTML/PDF using files under budapest/images."
        ),
    )
    parser.add_argument(
        "--budapest-root",
        type=Path,
        default=_PROJECT_ROOT / "budapest",
        help="Budapest tree root (default: budapest/)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Output directory (default: budapest/output/)",
    )
    parser.add_argument(
        "--html-only",
        action="store_true",
    ...
    budapest_root = args.budapest_root.resolve()
    out_dir = (
        args.output_dir.resolve()
        if args.output_dir
        else budapest_root / "output"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    places = _places_with_local_images(budapest_root)
    if not places:
        print(
            "No places with local images (>= {} bytes). "
            "Fill budapest_places.json and run "
            "scripts/download_budapest_images.py.".format(MIN_IMAGE_BYTES),
            file=sys.stderr,
        )
        return 2

    html_path = out_dir / BUDAPEST_HTML_NAME
    html_path.write_text(_build_html(budapest_root, places), encoding="utf-8")
    ...
    pdf_path = out_dir / BUDAPEST_PDF_NAME
    footer = (
    ...
    if _pdf_via_playwright(
        ...
        static_site_root=budapest_root,
    ):
