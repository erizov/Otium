# -*- coding: utf-8 -*-
"""Download New York City guide images (flat images/)."""
...
from new_york.whitelist import default_whitelist_path, url_is_whitelisted
...
_TITLE_PAGE_ASSETS: tuple[tuple[str, str], ...] = (
    (
        ...
        "Seal_of_New_York_City.svg",
    ),
    (
        "images/guide_flag.svg",
        "https://upload.wikimedia.org/wikipedia/commons/b/ba/"
        "Flag_of_New_York_City.svg",
    ),
)
_USER_AGENT = (
    "ExcursionGuide-NewYorkCity/1.0 (batch download; Python urllib)"
)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Download New York City guide images into new_york/images/."
        ),
    )
    parser.add_argument(
        "--new-york-root",
    ...
    root = args.new_york_root.resolve()
    ...
    if not todo:
        n = len(NEW_YORK_PLACES)
        if n == 0:
            print(
                "No entries in NEW_YORK_PLACES — add new_york_places.json.",
                file=sys.stderr,
            )
