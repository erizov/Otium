...
from florence.data.places_registry import FLORENCE_PLACES, FlorencePlace
...
FLORENCE_HTML_NAME = "florence_guide.html"
FLORENCE_PDF_NAME = "florence_guide.pdf"

_TITLE_SYMBOLS: tuple[tuple[str, str], ...] = (
    (
        "images/guide_coat_of_arms.svg",
        "Coat of arms of Florence (Wikimedia Commons)",
    ),
    (
        "images/guide_flag.svg",
        "Flag of Florence (Wikimedia Commons)",
    ),
)
...
def _place_has_displayable_body(p: FlorencePlace) -> bool:
...
def _places_with_local_images(root: Path) -> list[FlorencePlace]:
    out: list[FlorencePlace] = []
    for p in FLORENCE_PLACES:
...
def _place_meta_line(p: FlorencePlace) -> str | None:
    parts: list[str] = []
...
def _image_srcs_for_place(root: Path, p: FlorencePlace) -> list[str]:
    if p.get("suppress_images_for_pdf"):
        return []
    out: list[str] = []
    rel = p.get("image_rel_path")
...
def _place_block(p: FlorencePlace, img_srcs: list[str]) -> str:
    name_en = escape(p.get("name_en", ""))
    name_plain = p.get("name_en", "")
    sub_it = p.get("subtitle_it", "")
    sub_html = (
        '<p class="sub-it">{}</p>'.format(escape(sub_it))
        if is_substantive_text(sub_it)
        else ""
    )
...
def _heraldry_html(root: Path) -> str:
    chunks: list[str] = [
        '<div class="florence-title-symbols" '
        'aria-label="Florence coat of arms and flag">',
...
def _build_html(root: Path, places: list[FlorencePlace]) -> str:
    font_href, _, _ = typography_triple("florence")
    blocks: list[str] = [_cover_otium_html()]
    blocks.append(
        '<header class="guide-title">'
        "{}"
        "<h1 class=\"guide-title-main\">Florence</h1>"
    ...
    body_inner = "\n".join(blocks)
    css = guide_inline_css("florence-title-symbols", "florence")
    return (
        "<!DOCTYPE html>\n"
        '<html lang="en">\n<head>\n'
        '<meta charset="utf-8"/>\n'
        "<title>Guide · Florence · OTIUM</title>\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build Florence HTML/PDF using files under florence/images."
        ),
    )
    parser.add_argument(
        "--florence-root",
        type=Path,
        default=_PROJECT_ROOT / "florence",
        help="Florence tree root (default: florence/)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Output directory (default: florence/output/)",
    )
    parser.add_argument(
        "--html-only",
        action="store_true",
    ...
    florence_root = args.florence_root.resolve()
    out_dir = (
        args.output_dir.resolve()
        if args.output_dir
        else florence_root / "output"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    places = _places_with_local_images(florence_root)
    if not places:
        print(
            "No places with local images (>= {} bytes). "
            "Fill florence_places.json and run "
            "scripts/download_florence_images.py.".format(MIN_IMAGE_BYTES),
            file=sys.stderr,
        )
        return 2

    html_path = out_dir / FLORENCE_HTML_NAME
    html_path.write_text(_build_html(florence_root, places), encoding="utf-8")
    ...
    pdf_path = out_dir / FLORENCE_PDF_NAME
    footer = (
    ...
    if _pdf_via_playwright(
        ...
        static_site_root=florence_root,
    ):
