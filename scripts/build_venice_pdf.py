...
from venice.data.places_registry import VENICE_PLACES, VenicePlace
...
VENICE_HTML_NAME = "venice_guide.html"
VENICE_PDF_NAME = "venice_guide.pdf"

_TITLE_SYMBOLS: tuple[tuple[str, str], ...] = (
    (
        "images/guide_coat_of_arms.svg",
        "Coat of arms of the Republic of Venice (Wikimedia Commons)",
    ),
    (
        "images/guide_flag.svg",
        "Flag of the Republic of Venice (Wikimedia Commons)",
    ),
)
...
def _place_has_displayable_body(p: VenicePlace) -> bool:
...
def _places_with_local_images(root: Path) -> list[VenicePlace]:
    out: list[VenicePlace] = []
    for p in VENICE_PLACES:
...
def _place_meta_line(p: VenicePlace) -> str | None:
    parts: list[str] = []
...
def _image_srcs_for_place(root: Path, p: VenicePlace) -> list[str]:
    if p.get("suppress_images_for_pdf"):
        return []
    out: list[str] = []
    rel = p.get("image_rel_path")
...
def _place_block(p: VenicePlace, img_srcs: list[str]) -> str:
...
def _heraldry_html(root: Path) -> str:
    chunks: list[str] = [
        '<div class="venice-title-symbols" '
        'aria-label="Venice historical coat of arms and flag">',
...
def _build_html(root: Path, places: list[VenicePlace]) -> str:
    font_href, _, _ = typography_triple("venice")
    blocks: list[str] = [_cover_otium_html()]
    blocks.append(
        '<header class="guide-title">'
        "{}"
        "<h1 class=\"guide-title-main\">Venice</h1>"
    ...
    body_inner = "\n".join(blocks)
    css = guide_inline_css("venice-title-symbols", "venice")
    return (
        "<!DOCTYPE html>\n"
        '<html lang="en">\n<head>\n'
        '<meta charset="utf-8"/>\n'
        "<title>Guide · Venice · OTIUM</title>\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build Venice HTML/PDF using files under venice/images."
        ),
    )
    parser.add_argument(
        "--venice-root",
        type=Path,
        default=_PROJECT_ROOT / "venice",
        help="Venice tree root (default: venice/)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Output directory (default: venice/output/)",
    )
    parser.add_argument(
        "--html-only",
        action="store_true",
    ...
    venice_root = args.venice_root.resolve()
    out_dir = (
        args.output_dir.resolve()
        if args.output_dir
        else venice_root / "output"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    places = _places_with_local_images(venice_root)
    if not places:
        print(
            "No places with local images (>= {} bytes). "
            "Fill venice_places.json and run "
            "scripts/download_venice_images.py.".format(MIN_IMAGE_BYTES),
            file=sys.stderr,
        )
        return 2

    html_path = out_dir / VENICE_HTML_NAME
    html_path.write_text(_build_html(venice_root, places), encoding="utf-8")
    ...
    pdf_path = out_dir / VENICE_PDF_NAME
    footer = (
