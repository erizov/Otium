...
from bangkok.data.places_registry import BANGKOK_PLACES
from bangkok.whitelist import default_whitelist_path
from bangkok.whitelist import url_is_whitelisted
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download bangkok guide images.",
    )
    parser.add_argument(
        "--bangkok-root",
        type=Path,
        default=_PROJECT_ROOT / "bangkok",
        dest="city_root",
        help="bangkok tree root",
    )
    add_download_image_args(parser)
    args = parser.parse_args()
    return download_jerusalem_style_images(
        city_root=args.city_root,
        places=BANGKOK_PLACES,
        whitelist_path=default_whitelist_path(),
        title_page_assets=title_page_assets_for_download_arg("bangkok"),
        args=args,
