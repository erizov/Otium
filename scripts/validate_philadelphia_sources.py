# -*- coding: utf-8 -*-
"""Validate Philadelphia image_source_url against the whitelist."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate Philadelphia place image_source_url against "
    ...
    if errors:
        print("Philadelphia source URL validation failed:", file=sys.stderr)
        for line in errors:
            print("  {}".format(line), file=sys.stderr)
        return 1
    print("OK: {} place(s), all image_source_url whitelisted.".format(
        len(PHILADELPHIA_PLACES),
    ))
    return 0
