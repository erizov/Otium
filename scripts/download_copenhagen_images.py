# -*- coding: utf-8 -*-
"""Download copenhagen guide images."""
...
from copenhagen.whitelist import default_whitelist_path
from copenhagen.whitelist import url_is_whitelisted
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download copenhagen guide images.",
    )
    parser.add_argument(
        "--copenhagen-root",
        type=Path,
        default=_PROJECT_ROOT / "copenhagen",
        dest="city_root",
        help="copenhagen tree root",
    )
    add_download_image_args(parser)
    args = parser.parse_args()
    return download_jerusalem_style_images(
        city_root=args.city_root,
        places=COPENHAGEN_PLACES,
        whitelist_path=default_whitelist_path(),
        title_page_assets=title_page_assets_for_download_arg("copenhagen"),
        args=args,
