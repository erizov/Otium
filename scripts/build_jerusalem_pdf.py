...
from jerusalem.data.places_registry import JERUSALEM_PLACES, JerusalemPlace
...
JERUSALEM_HTML_NAME = "jerusalem_guide.html"
JERUSALEM_PDF_NAME = "jerusalem_guide.pdf"

_TITLE_SYMBOLS: tuple[tuple[str, str], ...] = (
    (
        "images/guide_coat_of_arms.svg",
        "Emblem of Jerusalem (Wikimedia Commons)",
    ),
    (
        "images/guide_flag.svg",
        "Flag of Jerusalem (Wikimedia Commons)",
    ),
)
...
def _place_has_displayable_body(p: JerusalemPlace) -> bool:
...
def _places_with_local_images(root: Path) -> list[JerusalemPlace]:
    out: list[JerusalemPlace] = []
    for p in JERUSALEM_PLACES:
...
def _place_meta_line(p: JerusalemPlace) -> str | None:
    parts: list[str] = []
...
def _image_srcs_for_place(root: Path, p: JerusalemPlace) -> list[str]:
    if p.get("suppress_images_for_pdf"):
        return []
    out: list[str] = []
    rel = p.get("image_rel_path")
...
def _place_block(p: JerusalemPlace, img_srcs: list[str]) -> str:
    name_en = escape(p.get("name_en", ""))
    name_plain = p.get("name_en", "")
    sub_he = p.get("subtitle_he", "")
    sub_html = (
        '<p class="sub-he" dir="rtl" lang="he">{}</p>'.format(escape(sub_he))
        if is_substantive_text(sub_he)
        else ""
    )
...
def _heraldry_html(root: Path) -> str:
    chunks: list[str] = [
        '<div class="jerusalem-title-symbols" '
        'aria-label="Jerusalem emblem and flag">',
...
def _build_html(root: Path, places: list[JerusalemPlace]) -> str:
    font_href, _, _ = typography_triple("jerusalem")
    blocks: list[str] = [_cover_otium_html()]
    blocks.append(
        '<header class="guide-title">'
        "{}"
        "<h1 class=\"guide-title-main\">Jerusalem</h1>"
    ...
    body_inner = "\n".join(blocks)
    css = guide_inline_css("jerusalem-title-symbols", "jerusalem")
    return (
        "<!DOCTYPE html>\n"
        '<html lang="en">\n<head>\n'
        '<meta charset="utf-8"/>\n'
        "<title>Guide · Jerusalem · OTIUM</title>\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build Jerusalem HTML/PDF using files under jerusalem/images."
        ),
    )
    parser.add_argument(
        "--jerusalem-root",
        type=Path,
        default=_PROJECT_ROOT / "jerusalem",
        help="Jerusalem tree root (default: jerusalem/)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Output directory (default: jerusalem/output/)",
    )
    parser.add_argument(
        "--html-only",
        action="store_true",
    ...
    jerusalem_root = args.jerusalem_root.resolve()
    out_dir = (
        args.output_dir.resolve()
        if args.output_dir
        else jerusalem_root / "output"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    places = _places_with_local_images(jerusalem_root)
    if not places:
        print(
            "No places with local images (>= {} bytes). "
            "Fill jerusalem_places.json and run "
            "scripts/download_jerusalem_images.py.".format(MIN_IMAGE_BYTES),
            file=sys.stderr,
        )
        return 2

    html_path = out_dir / JERUSALEM_HTML_NAME
    html_path.write_text(_build_html(jerusalem_root, places), encoding="utf-8")
    ...
    pdf_path = out_dir / JERUSALEM_PDF_NAME
    footer = (
    ...
    if _pdf_via_playwright(
        ...
        static_site_root=jerusalem_root,
    ):
