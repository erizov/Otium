# -*- coding: utf-8 -*-
"""Validate Barcelona image_source_url entries against the whitelist."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate Barcelona place image_source_url against "
    ...
    if errors:
        print("Barcelona source URL validation failed:", file=sys.stderr)
        for line in errors:
            print("  {}".format(line), file=sys.stderr)
        return 1
    print("OK: {} place(s), all image_source_url whitelisted.".format(
        len(BARCELONA_PLACES),
    ))
    return 0
