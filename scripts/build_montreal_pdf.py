...
from montreal.data.places_registry import MONTREAL_PLACES, MontrealPlace
...
MONTREAL_HTML_NAME = "montreal_guide.html"
MONTREAL_PDF_NAME = "montreal_guide.pdf"

_TITLE_SYMBOLS: tuple[tuple[str, str], ...] = (
    (
        "images/guide_coat_of_arms.svg",
        "Coat of arms of Montreal (Wikimedia Commons)",
    ),
    (
        "images/guide_flag.svg",
        "Flag of Montreal (Wikimedia Commons)",
    ),
)
...
def _place_has_displayable_body(p: MontrealPlace) -> bool:
...
def _places_with_local_images(root: Path) -> list[MontrealPlace]:
    out: list[MontrealPlace] = []
    for p in MONTREAL_PLACES:
...
def _place_meta_line(p: MontrealPlace) -> str | None:
    parts: list[str] = []
...
def _image_srcs_for_place(root: Path, p: MontrealPlace) -> list[str]:
    if p.get("suppress_images_for_pdf"):
        return []
    out: list[str] = []
    rel = p.get("image_rel_path")
...
def _place_block(p: MontrealPlace, img_srcs: list[str]) -> str:
    name_en = escape(p.get("name_en", ""))
    name_plain = p.get("name_en", "")
    sub_fr = p.get("subtitle_fr", "")
    sub_html = (
        '<p class="sub-fr">{}</p>'.format(escape(sub_fr))
        if is_substantive_text(sub_fr)
        else ""
    )
...
def _heraldry_html(root: Path) -> str:
    chunks: list[str] = [
        '<div class="montreal-title-symbols" '
        'aria-label="Montreal coat of arms and flag">',
...
def _build_html(root: Path, places: list[MontrealPlace]) -> str:
    font_href, _, _ = typography_triple("montreal")
    blocks: list[str] = [_cover_otium_html()]
    blocks.append(
        '<header class="guide-title">'
        "{}"
        "<h1 class=\"guide-title-main\">Montreal</h1>"
    ...
    body_inner = "\n".join(blocks)
    css = guide_inline_css("montreal-title-symbols", "montreal")
    return (
        "<!DOCTYPE html>\n"
        '<html lang="en">\n<head>\n'
        '<meta charset="utf-8"/>\n'
        "<title>Guide · Montreal · OTIUM</title>\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build Montreal HTML/PDF using files under montreal/images."
        ),
    )
    parser.add_argument(
        "--montreal-root",
        type=Path,
        default=_PROJECT_ROOT / "montreal",
        help="Montreal tree root (default: montreal/)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Output directory (default: montreal/output/)",
    )
    parser.add_argument(
        "--html-only",
        action="store_true",
    ...
    montreal_root = args.montreal_root.resolve()
    out_dir = (
        args.output_dir.resolve()
        if args.output_dir
        else montreal_root / "output"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    places = _places_with_local_images(montreal_root)
    if not places:
        print(
            "No places with local images (>= {} bytes). "
            "Fill montreal_places.json and run "
            "scripts/download_montreal_images.py.".format(MIN_IMAGE_BYTES),
            file=sys.stderr,
        )
        return 2

    html_path = out_dir / MONTREAL_HTML_NAME
    html_path.write_text(_build_html(montreal_root, places), encoding="utf-8")
    ...
    pdf_path = out_dir / MONTREAL_PDF_NAME
    footer = (
    ...
    if _pdf_via_playwright(
        ...
        static_site_root=montreal_root,
    ):
