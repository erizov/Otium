# -*- coding: utf-8 -*-
"""Download Barcelona guide images from image_source_url (flat images/)."""
...
from barcelona.whitelist import default_whitelist_path, url_is_whitelisted
...
_USER_AGENT = (
    "ExcursionGuide-Barcelona/1.0 (batch download for local guide; "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download Barcelona guide images into barcelona/images/.",
    )
    parser.add_argument(
        "--barcelona-root",
        type=Path,
    ...
    root = args.barcelona_root.resolve()
    ...
    if not todo:
        n = len(BARCELONA_PLACES)
        if n == 0:
            print(
                "No entries in BARCELONA_PLACES — add barcelona_places.json.",
                file=sys.stderr,
            )
