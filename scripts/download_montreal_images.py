# -*- coding: utf-8 -*-
"""Download Montreal guide images from image_source_url (flat images/)."""
...
from montreal.whitelist import default_whitelist_path, url_is_whitelisted
...
_TITLE_PAGE_ASSETS: tuple[tuple[str, str], ...] = (
    (
        ...
        "https://upload.wikimedia.org/wikipedia/commons/4/47/Coat_of_arms_of_Montreal.svg",
    ),
    (
        "images/guide_flag.svg",
        "https://upload.wikimedia.org/wikipedia/commons/d/dc/Flag_of_Montreal.svg",
    ),
)
_USER_AGENT = (
    "ExcursionGuide-Montreal/1.0 (batch download for local guide; "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download Montreal guide images into montreal/images/.",
    )
    parser.add_argument(
        "--montreal-root",
        type=Path,
    ...
    root = args.montreal_root.resolve()
    ...
    if not todo:
        n = len(MONTREAL_PLACES)
        if n == 0:
            print(
                "No entries in MONTREAL_PLACES — add montreal_places.json.",
                file=sys.stderr,
            )
