# -*- coding: utf-8 -*-
"""Validate Jerusalem image_source_url entries against the whitelist."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate Jerusalem place image_source_url against "
    ...
    if errors:
        print("Jerusalem source URL validation failed:", file=sys.stderr)
        for line in errors:
            print("  {}".format(line), file=sys.stderr)
        return 1
    print("OK: {} place(s), all image_source_url whitelisted.".format(
        len(JERUSALEM_PLACES),
    ))
    return 0
