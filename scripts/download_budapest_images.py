# -*- coding: utf-8 -*-
"""Download Budapest guide images from image_source_url (flat images/)."""
...
from budapest.whitelist import default_whitelist_path, url_is_whitelisted
...
_TITLE_PAGE_ASSETS: tuple[tuple[str, str], ...] = (
    (
        ...
        "Coa_Hungary_Town_Budapest_big.svg",
    ),
    (
        "images/guide_flag.svg",
        "https://upload.wikimedia.org/wikipedia/commons/d/d5/"
        "Flag_of_Budapest_%282011-%29.svg",
    ),
)
_USER_AGENT = (
    "ExcursionGuide-Budapest/1.0 (batch download for local guide; "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download Budapest guide images into budapest/images/.",
    )
    parser.add_argument(
        "--budapest-root",
        type=Path,
    ...
    root = args.budapest_root.resolve()
    ...
    if not todo:
        n = len(BUDAPEST_PLACES)
        if n == 0:
            print(
                "No entries in BUDAPEST_PLACES — add budapest_places.json.",
                file=sys.stderr,
            )
