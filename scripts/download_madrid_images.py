# -*- coding: utf-8 -*-
"""Download Madrid guide images from image_source_url (flat images/)."""
...
from madrid.data.places_registry import MADRID_PLACES
from madrid.whitelist import default_whitelist_path, url_is_whitelisted
...
_TITLE_PAGE_ASSETS: tuple[tuple[str, str], ...] = (
    (
        ...
        "Coat_of_Arms_of_Madrid_City.svg",
    ),
    (
        "images/guide_flag.svg",
        "https://upload.wikimedia.org/wikipedia/commons/9/9c/"
        "Flag_of_the_Community_of_Madrid.svg",
    ),
)
_USER_AGENT = (
    "ExcursionGuide-Madrid/1.0 (batch download for local guide; "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download Madrid guide images into madrid/images/.",
    )
    parser.add_argument(
        "--madrid-root",
        type=Path,
    ...
    if not todo:
        ...
        if n == 0:
            print(
                "No entries in MADRID_PLACES — add madrid_places.json.",
                file=sys.stderr,
            )
