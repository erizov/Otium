# -*- coding: utf-8 -*-
"""HTML + PDF for Rome from files under rome/images/."""
...
from rome.data.places_registry import ROME_PLACES, RomePlace
...
ROME_HTML_NAME = "rome_guide.html"
ROME_PDF_NAME = "rome_guide.pdf"

_TITLE_SYMBOLS: tuple[tuple[str, str], ...] = (
    (
        "images/guide_coat_of_arms.svg",
        "Coat of arms of Rome (Wikimedia Commons)",
    ),
    (
        "images/guide_flag.svg",
        "Flag of Rome (Wikimedia Commons)",
    ),
)
...
def _place_has_displayable_body(p: RomePlace) -> bool:
...
def _places_with_local_images(root: Path) -> list[RomePlace]:
    out: list[RomePlace] = []
    for p in ROME_PLACES:
...
def _place_meta_line(p: RomePlace) -> str | None:
    parts: list[str] = []
...
def _image_srcs_for_place(root: Path, p: RomePlace) -> list[str]:
    if p.get("suppress_images_for_pdf"):
        return []
    out: list[str] = []
    rel = p.get("image_rel_path")
...
def _place_block(p: RomePlace, img_srcs: list[str]) -> str:
...
def _heraldry_html(root: Path) -> str:
    chunks: list[str] = [
        '<div class="rome-title-symbols" '
        'aria-label="Rome coat of arms and flag">',
...
def _build_html(root: Path, places: list[RomePlace]) -> str:
    font_href, _, _ = typography_triple("rome")
    blocks: list[str] = [_cover_otium_html()]
    blocks.append(
        '<header class="guide-title">'
        "{}"
        "<h1 class=\"guide-title-main\">Rome</h1>"
    ...
    body_inner = "\n".join(blocks)
    css = guide_inline_css("rome-title-symbols", "rome")
    return (
        "<!DOCTYPE html>\n"
        '<html lang="en">\n<head>\n'
        '<meta charset="utf-8"/>\n'
        "<title>Guide · Rome · OTIUM</title>\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build Rome HTML/PDF using files under rome/images."
        ),
    )
    parser.add_argument(
        "--rome-root",
        type=Path,
        default=_PROJECT_ROOT / "rome",
        help="Rome tree root (default: rome/)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Output directory (default: rome/output/)",
    )
    parser.add_argument(
        "--html-only",
        action="store_true",
    ...
    rome_root = args.rome_root.resolve()
    out_dir = (
        args.output_dir.resolve()
        if args.output_dir
        else rome_root / "output"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    places = _places_with_local_images(rome_root)
    if not places:
        print(
            "No places with local images (>= {} bytes). "
            "Fill rome_places.json and run "
            "scripts/download_rome_images.py.".format(MIN_IMAGE_BYTES),
            file=sys.stderr,
        )
        return 2

    html_path = out_dir / ROME_HTML_NAME
    html_path.write_text(_build_html(rome_root, places), encoding="utf-8")
    ...
    pdf_path = out_dir / ROME_PDF_NAME
    footer = (
