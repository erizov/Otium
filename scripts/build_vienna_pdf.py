# -*- coding: utf-8 -*-
"""HTML + PDF for Vienna from files under vienna/images/."""
...
from vienna.data.places_registry import VIENNA_PLACES, ViennaPlace
...
VIENNA_HTML_NAME = "vienna_guide.html"
VIENNA_PDF_NAME = "vienna_guide.pdf"

_TITLE_SYMBOLS: tuple[tuple[str, str], ...] = (
    (
        "images/guide_coat_of_arms.svg",
        "Coat of arms of Vienna (Wikimedia Commons)",
    ),
    (
        "images/guide_flag.svg",
        "Flag of Vienna state (Wikimedia Commons)",
    ),
)
...
def _place_has_displayable_body(p: ViennaPlace) -> bool:
...
def _places_with_local_images(root: Path) -> list[ViennaPlace]:
    out: list[ViennaPlace] = []
    for p in VIENNA_PLACES:
...
def _image_srcs_for_place(root: Path, p: ViennaPlace) -> list[str]:
    if p.get("suppress_images_for_pdf"):
        return []
    out: list[str] = []
    rel = p.get("image_rel_path")
...
def _heraldry_html(root: Path) -> str:
    chunks: list[str] = [
        '<div class="vienna-title-symbols" '
        'aria-label="Vienna city arms and flag">',
...
def _build_html(root: Path, places: list[ViennaPlace]) -> str:
    font_href, _, _ = typography_triple("vienna")
    blocks: list[str] = [_cover_otium_html()]
    blocks.append(
        '<header class="guide-title">'
        "{}"
        "<h1 class=\"guide-title-main\">Vienna</h1>"
    ...
    body_inner = "\n".join(blocks)
    css = guide_inline_css("vienna-title-symbols", "vienna")
    return (
        "<!DOCTYPE html>\n"
        '<html lang="en">\n<head>\n'
        '<meta charset="utf-8"/>\n'
        "<title>Guide · Vienna · OTIUM</title>\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build Vienna HTML/PDF using files under vienna/images."
        ),
    )
    parser.add_argument(
        "--vienna-root",
        type=Path,
        default=_PROJECT_ROOT / "vienna",
        help="Vienna tree root (default: vienna/)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Output directory (default: vienna/output/)",
    )
    parser.add_argument(
        "--html-only",
        action="store_true",
    ...
    vienna_root = args.vienna_root.resolve()
    out_dir = (
        args.output_dir.resolve()
        if args.output_dir
        else vienna_root / "output"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    places = _places_with_local_images(vienna_root)
    if not places:
        print(
            "No places with local images (>= {} bytes). "
            "Fill vienna_places.json and run "
            "scripts/download_vienna_images.py.".format(MIN_IMAGE_BYTES),
            file=sys.stderr,
        )
        return 2

    html_path = out_dir / VIENNA_HTML_NAME
    html_path.write_text(_build_html(vienna_root, places), encoding="utf-8")
    ...
    pdf_path = out_dir / VIENNA_PDF_NAME
    footer = (
