# -*- coding: utf-8 -*-
"""Download Paris guide images from image_source_url (flat images/)."""
...
from paris.data.places_registry import PARIS_PLACES
from paris.whitelist import default_whitelist_path, url_is_whitelisted
...
_USER_AGENT = (
    "ExcursionGuide-Paris/1.0 (batch download for local guide; "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download Paris guide images into paris/images/.",
    )
    parser.add_argument(
        "--paris-root",
        type=Path,
    ...
    if not todo:
        ...
        if n == 0:
            print(
                "No entries in PARIS_PLACES — add paris_places.json.",
                file=sys.stderr,
            )
