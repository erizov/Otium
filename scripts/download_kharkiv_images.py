...
from kharkiv.whitelist import default_whitelist_path
from kharkiv.whitelist import url_is_whitelisted
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download Kharkiv guide images.",
    )
    parser.add_argument(
        "--kharkiv-root",
        type=Path,
        default=_PROJECT_ROOT / "kharkiv",
        dest="city_root",
        help="kharkiv tree root",
    )
    add_download_image_args(parser)
    args = parser.parse_args()
    return download_jerusalem_style_images(
        city_root=args.city_root,
        places=KHARKIV_PLACES,
        whitelist_path=default_whitelist_path(),
        title_page_assets=title_page_assets_for_download_arg("kharkiv"),
        args=args,
