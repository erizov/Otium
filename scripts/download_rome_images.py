# -*- coding: utf-8 -*-
"""Download Rome guide images from image_source_url (flat images/)."""
...
from rome.data.places_registry import ROME_PLACES
from rome.whitelist import default_whitelist_path, url_is_whitelisted
...
_USER_AGENT = (
    "ExcursionGuide-Rome/1.0 (batch download for local guide; "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download Rome guide images into rome/images/.",
    )
    parser.add_argument(
        "--rome-root",
        type=Path,
    ...
    if not todo:
        ...
        if n == 0:
            print(
                "No entries in ROME_PLACES — add rome_places.json.",
                file=sys.stderr,
            )
